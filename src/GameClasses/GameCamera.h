...
public: 
    GameCamera(
        ...
        float zoom = 1.0f) :
            _position(pos), _size(size), _origin(origin), _zoom(zoom) {}
    ...
    glm::vec2 ret = (pos - GetPosition()) * GetZoom() + (GetSize() / 2.0f);
    return ret;
}

    inline glm::vec2 ScreenToWorld(const glm::ivec2 &pos) {
        glm::vec2 ret = glm::vec2(pos - glm::ivec2((GetSize() / 2.0f))) / GetZoom() + GetPosition();
        return ret;
    }
    ...
    inline void RectWorldToScreen(const SDL_FRect* in, SDL_FRect* out) {
        ...
        glm::vec2 wh = glm::vec2(in->w, in->h) * (GetZoom());
